...
router.get('/:id/reports/monthly-counts', authMiddleware, ownerOrSalesOrTeamLeaderOrAccountManagerOrModeratorOrContentCreator, async (req: AuthRequest, res: Response): Promise<void> => {
  const clientId = String(req.params.id);
  const month = String(req.query.month || new Date().toISOString().substring(0, 7));
  try {
    const counts = await calculateMonthlyDeliverables(clientId, month);
...
router.get('/:id/reports', authMiddleware, ownerOrSalesOrTeamLeaderOrAccountManagerOrModeratorOrContentCreator, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    const reportsWithAutoCounts = await Promise.all(
      (data || []).map(async (rep: any) => {
        const ym = rep.report_month ? String(rep.report_month).substring(0, 7) : '';
        const dynamicCounts = await calculateMonthlyDeliverables(clientId, ym);
        return {
          ...rep,
...
router.post('/:id/reports', authMiddleware, ownerOrSalesOrTeamLeaderOrAccountManager, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    const autoCounts = await calculateMonthlyDeliverables(clientId, ym);
