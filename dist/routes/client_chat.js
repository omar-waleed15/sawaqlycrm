...
router.get('/rooms', auth_1.authMiddleware, staffOnly, async (req, res) => {
    try {
        ...
        const rooms = await Promise.all((clients || []).map(async (client) => {
            const { data: lastMsg, error: msgErr } = await supabase_1.supabaseAdmin
...
router.get('/rooms/:client_id/messages', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const { data: messages, error: messagesErr } = await supabase_1.supabaseAdmin
            .from('client_messages')
...
router.post('/rooms/:client_id/messages', auth_1.authMiddleware, async (req, res) => {
    const { client_id } = req.params;
    ...
    try {
        ...
        // Insert message
        const { data: message, error: insertErr } = await supabase_1.supabaseAdmin
            .from('client_messages')
            .insert({
            client_id,
            sender_id: userId,
