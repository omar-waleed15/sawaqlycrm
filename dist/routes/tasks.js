    ...
    try {
        ...
        if (isTaskAdmin(userRole)) {
            ...
            if (userRole === 'content_creator') {
                const { data: internAssignments } = await supabase_1.supabaseAdmin
                    .from('task_assignees')
                    .select('task_id, user:profiles!inner(role)')
                    .eq('user.role', 'content_creator_intern');
                const internTaskIds = (internAssignments || []).map((a) => a.task_id);
...
router.post('/', auth_1.authMiddleware, roleCheck_1.ownerOrTeamLeaderOrSalesOrContentCreator, async (req, res) => {
    ...
    if (req.user.role === 'content_creator' && Array.isArray(assignee_ids) && assignee_ids.length > 0) {
        const { data: assigneeProfiles } = await supabase_1.supabaseAdmin
            .from('profiles')
            .select('id, role')
            .in('id', assignee_ids);
        const invalidAssignees = (assigneeProfiles || []).filter((p) => p.role !== 'content_creator_intern');
      ...
      `)
        ...
        if (!isTaskAdmin(req.user.role)) {
            ...
            const isContentCreatorManagingIntern = req.user.role === 'content_creator' &&
                data.task_assignees?.some((a) => a.user?.role === 'content_creator_intern');
...
router.delete('/:id', auth_1.authMiddleware, roleCheck_1.ownerOrTeamLeaderOrSalesOrContentCreator, async (req, res) => {
    const { id } = req.params;
    if (!(await canAdministerTask(req.user.id, req.user.role, id))) {
        res.status(403).json({ error: 'Access denied. You cannot administer this task if you are assigned to it.' });
        return;
...
router.post('/target', auth_1.authMiddleware, roleCheck_1.ownerOnly, async (req, res) => {
    ...
    try {
        const { data, error } = await supabase_1.supabaseAdmin
            .from('task_targets')
            .upsert({ user_id, target_tasks: Number(target_tasks), month }, { onConflict: 'user_id,month' })
            .select()
            .single();
        if (error) {
            res.status(500).json({ error: error.message });
...
router.get('/target/:userId/:month/progress', auth_1.authMiddleware, async (req, res) => {
    const { userId, month } = req.params;
    const userIdStr = userId;
    const monthStr = month;
    const callerId = req.user.id;
    ...
    try {
        ...
        const { data: targetData, error: targetError } = await supabase_1.supabaseAdmin
            .from('task_targets')
            .select('target_tasks')
            .eq('user_id', userIdStr)
            .eq('month', monthStr)
...
router.post('/:id/timer/start', auth_1.authMiddleware, async (req, res) => {
    const { id } = req.params;
    const callerId = req.user.id;
    try {
        const { data: assignment, error: aErr } = await supabase_1.supabaseAdmin
...
router.post('/:id/timer/stop', auth_1.authMiddleware, async (req, res) => {
    const { id } = req.params;
    const callerId = req.user.id;
    try {
        const { data: assignment, error: aErr } = await supabase_1.supabaseAdmin
