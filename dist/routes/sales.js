...
router.get('/dashboard', auth_1.authMiddleware, roleCheck_1.ownerOrSales, async (req, res) => {
    ...
    try {
        ...
        // 4. Fetch call logs
        let callLogsQuery = supabase_1.supabaseAdmin
...
function parseCairoTimeToISO(dateStr) {
    if (!dateStr)
        return null;
...
router.post('/leads/:leadId/calls', auth_1.authMiddleware, roleCheck_1.ownerOrSales, async (req, res) => {
    ...
    try {
        ...
        if (outcome === 'meeting_scheduled') {
            if (meeting_date)
                updates.meeting_date = parseCairoTimeToISO(meeting_date);
            if (Array.isArray(meeting_attendees))
                updates.meeting_attendees = meeting_attendees;
            if (meeting_notes !== undefined)
                updates.meeting_notes = meeting_notes || null;
        }
        ...
        if (outcome === 'meeting_scheduled' && Array.isArray(meeting_attendees) && meeting_attendees.length > 0) {
            ...
            const reminderRows = meeting_attendees.map((attendeeId) => ({
                sender_id: salesRepId,
                receiver_id: attendeeId,
                content: reminderText,
            }));
            try {
                await supabase_1.supabaseAdmin.from('reminders').insert(reminderRows);
            }
            catch (remErr) {
...
router.post('/leads/:leadId/close-won', auth_1.authMiddleware, roleCheck_1.ownerOrSales, async (req, res) => {
    ...
    try {
        ...
        if (name && amount && Number(amount) > 0) {
            const { data: contractData, error: contractErr } = await supabase_1.supabaseAdmin
                .from('contracts')
                .insert({
                client_id: leadId,
                name,
...
router.get('/calendar-events', auth_1.authMiddleware, roleCheck_1.ownerOrSales, async (req, res) => {
    try {
        const isOwner = req.user.role === 'owner' || req.user.role === 'team_leader';
        ...
        let meetingsQuery = supabase_1.supabaseAdmin
            .from('clients')
            .select('*')
        ...
        if (meetingsErr) {
            res.status(500).json({ error: meetingsErr.message });
            return;
        }
        // 2. Fetch call logs
        ...
        if (callsErr) {
            res.status(500).json({ error: callsErr.message });
            return;
        }
        ...
        let salesReps = [];
        if (isOwner) {
            const { data: reps } = await supabase_1.supabaseAdmin
                .from('profiles')
