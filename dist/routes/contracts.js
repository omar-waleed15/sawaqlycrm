...
router.get('/finance-stats', auth_1.authMiddleware, roleCheck_1.ownerOrSales, async (_req, res) => {
    try {
        // 1. Total Clients
        const { count: totalClients, error: clientsError } = await supabase_1.supabaseAdmin
            .from('clients')
        ...
        // 2. Active Projects
        const { count: activeProjects, error: projectsError } = await supabase_1.supabaseAdmin
            .from('projects')
            .select('*', { count: 'exact', head: true })
            .eq('status', 'active');
        if (projectsError) {
        ...
        const { data: activeContracts, error: contractsError } = await supabase_1.supabaseAdmin
            .from('contracts')
        ...
        const { count: upcomingRenewalsCount, error: renewalError } = await supabase_1.supabaseAdmin
            .from('contracts')
            .select('*', { count: 'exact', head: true })
            .eq('status', 'active')
        ...
        try {
            ...
            if (!expError && !salError) {
                const totalSalaries = (salaries || []).reduce((sum, s) => {
                    const penaltySum = s.penalties ? s.penalties.reduce((pSum, p) => pSum + (Number(p.amount) || 0), 0) : 0;
    ...
    try {
      ...
      `)
        ...
        const contracts = (data || []).map((c) => ({
            ...c,
            installments: (c.installments || []).sort((a, b) => new Date(a.due_date).getTime() - new Date(b.due_date).getTime()),
        }));
        res.json({ contracts });
    }
    catch (err) {
...
router.post('/', auth_1.authMiddleware, roleCheck_1.ownerOrSales, async (req, res) => {
    ...
    try {
        ...
        if (!is_recurring && Array.isArray(installments) && installments.length > 0) {
            ...
            await supabase_1.supabaseAdmin.from('contract_installments').insert(rows);
        }
        // Refetch with all relations
        const { data: contractDetails } = await supabase_1.supabaseAdmin
            .from('contracts')
            .select(`
        *,
        client:clients(id, name, company, email, phone, status),
...
router.put('/:id', auth_1.authMiddleware, roleCheck_1.ownerOrSales, async (req, res) => {
    ...
    try {
        ...
        if (renewal_date !== undefined)
            updates.renewal_date = renewal_date || null;
        const { data, error } = await supabase_1.supabaseAdmin
            .from('contracts')
            .update(updates)
            .eq('id', id)
        ...
        if (Array.isArray(installments)) {
            ...
            await supabase_1.supabaseAdmin.from('contract_installments').delete().eq('contract_id', id);
            // Insert new ones
...
router.patch('/:id/installments/:iid/paid', auth_1.authMiddleware, roleCheck_1.ownerOrSales, async (req, res) => {
