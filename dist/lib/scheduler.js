...
exports.getTargetDateForMonth = getTargetDateForMonth;
exports.generateMonthlyTasksForClient = generateMonthlyTasksForClient;
exports.triggerMonthlyTasksGeneration = triggerMonthlyTasksGeneration;
 ...
 */
function getTargetDateForMonth(originalDateStr, year, month) {
 ...
 */
async function generateMonthlyTasksForClient(client, year, month) {
    if (!client || !client.id || client.status !== 'active')
        return;
    ...
    const types = ['posts', 'reels', 'stories', 'photos'];
    const typeMap = {
    ...
    for (const tKey of types) {
        ...
        for (let i = 0; i < targetCount; i++) {
            ...
            if (!originalDate)
                continue; // If not configured, skip
            ...
            const { data: existing, error: checkErr } = await supabase_1.supabaseAdmin
                .from('tasks')
                .select('id')
            ...
            // Create outline task
            const { error: insertErr } = await supabase_1.supabaseAdmin
                .from('tasks')
                .insert({
 ...
 */
async function triggerMonthlyTasksGeneration(year, month) {
    try {
